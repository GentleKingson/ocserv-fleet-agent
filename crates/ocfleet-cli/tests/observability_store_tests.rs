use ocfleet_cli::store::{
    AlertEventRecord, HealthSnapshotRecord, ObservabilityJobRecord, ObservabilityRunInsert,
    ProbeObservationInsert, RetentionPolicyRecord, Store,
};
...
#[test]
fn observability_store_tests_new_database_uses_schema_version_3() {
    let (_dir, store, _db) = open_temp_store();

    assert_eq!(store.current_schema_version().expect("version"), 3);
}
...
fn observability_store_tests_sets_and_gets_retention_policy() {
    ...
    let policy = RetentionPolicyRecord {
        scope: "probe_observations".to_string(),
    ...
    assert!(
        store
            .get_retention_policy("probe_observations")
...
fn observability_store_tests_migration_is_idempotent_when_reopening_database() {
    ...
    assert_eq!(first.current_schema_version().expect("first version"), 3);
    drop(first);

    let second = Store::open(&db).expect("second open");
    assert_eq!(second.current_schema_version().expect("second version"), 3);
}
