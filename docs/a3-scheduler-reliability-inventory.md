...
| Backend-neutral contract | claim, renewal, release, maintenance, run, outcome, and finish mutations are defined on `StoreWriter`; scheduler production code uses the trait boundary | compiler coverage for the SQLite `StoreWriter` implementation; controller mutation source guard | proven for contract; the experimental Postgres snapshot foundation remains C1 |
