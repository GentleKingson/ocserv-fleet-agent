...
| Bounded change set | At most 512 changes are emitted with exact total count and truncation. Truncated plans cannot be approved. Policy collections, files, plans, signatures, history, identifiers, and output fields are independently bounded. |
| Policy history | Private append-only JSONL history is capped at 256 unique revisions and 1 MiB; duplicate, malformed, unknown-field, timestamp, digest, and approval-link contamination fail closed. An exclusive cross-process file lock covers reread, duplicate check, and single-buffer append. |
...
| Approval record | `trust policy approve` emits a private Ed25519-signed record bound to exact plan hash, revision, global actor, timestamp, and key ID. Both approval and history resolve actor/key ID through an independently pinned reviewer keyring; the approval artifact contains no self-asserted public key. |
