...
| 3. Read-only API/OpenAPI | Complete | Added OpenAPI 3.1.1 for all 16 `GET` routes, typed envelopes/errors, viewer bearer authentication, strict identifiers/limits/windows, read-only SQLite validation, method rejection, and router/spec drift tests. |
...
| 7. Governance/RBAC | Implemented C2 API layer | Actor resolution fails closed for invalid explicit values. API authentication unifies local development, legacy bearer, expiring service accounts, pinned-JWKS EdDSA OIDC, trusted-proxy mTLS, and explicit one-hour break glass. Five fixed roles feed per-route default-deny permissions; issuer/audience/time/signature/rotation/mapping and mTLS/service permission tests pass. Auth failures expose only fixed metrics and redacted logs. The API remains GET-only and adds no agent RPC trigger. |
...
| 8a. Scheduler reliability | Operationally mature A3 | Schemas 19-20 add deterministic fenced claims, lease heartbeat, abandoned-run recovery, bounded misfire coalescing, transient-only retry/backoff under a hard RPC budget, atomically audited global maintenance suppression, audited SIGINT/SIGTERM admission-stop/drain/restart behavior, per-attempt timeouts, and deterministic bounded jitter. The completion inventory maps every A3 acceptance requirement to implementation and test evidence. |
| 8b. Independent health evaluator | Operationally mature A4 | Schema 21 and backend-neutral writers persist watermark/policy/computation identity, atomically audited lifecycle and snapshots, typed failures, and bounded abandoned-run recovery. The observation-only run and daemon commands update health independently of dashboard reads, coalesce one-minute input buckets, and drain/restart on SIGINT/SIGTERM without agent RPC or trust/node/scheduler mutation. The completion inventory maps every A4 requirement to exact evidence. |
| 8c. Automatic alert delivery | Implemented A5 slices | Schema 22, backend-neutral fenced queue writers, and one-shot/daemon workers add idempotent automatic webhook delivery, bounded retry/DLQ/recovery, attempt history, HMAC dispatch, per-group and repeat rate limits, typed preflight failures, and graceful shutdown. JSONL remains manual; no command, script, template, or remediation path exists. |
| 8d. Health history and SLOs | Operationally mature B1 | Schemas 23-25 add append-only health history, reproducible 5m/1h/1d rollups, one latest status per five-minute slot, fixed 24h/7d/30d CLI/API projections, independent retention, deterministic closed-bucket refresh, and a hardened network-isolated timer. The B1 inventory maps missing-data, duration, latency, error, certificate, drift, bounds, and read-only evidence. |
| 9. Experimental Postgres snapshot backend | Default-off foundation, not production deployable | The explicitly named Postgres-wrapped SQLite snapshot backend stores one complete checksummed SQLite image, globally serializes full-image writes, recommends at most 64 MiB, exposes persistent state revision and bounded timing metrics, and uses commit-time wall-clock lease revalidation. It provides import/export and fencing experiments, not native Postgres rows, concurrent-write scaling, or high-frequency observability ingestion. C3/C-READY remain required. SQLite and the SQLite-only API remain the defaults. |
| 10. Controlled writes | D0 implemented | Schema 28 and the default-off module add private trusted-key resolution, actor/reason-bound signed intents, replay-safe requests, default-deny dry-run policy decisions, durable expiry, explicit terminal transitions, expiring two-person approvals, cancellation, and atomic low-sensitive transition audit. Feature-enabled `change` CLI commands complete the workflow and expose no dispatch verb. The agent still rejects every write RPC and no reload/restart/apply adapter exists. |
...
- Complete transparent Postgres selection for legacy CLI/API consumers only
  after the backend-neutral projection contract can remain equally strict.
  Explicit Postgres lifecycle commands and the runtime store are default-off;
  SQLite remains the default legacy command/API backend.
