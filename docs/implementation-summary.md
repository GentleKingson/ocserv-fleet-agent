...
| 8b. Independent health evaluator | Implemented A4 slices | Schema 21 and backend-neutral writers persist watermark/policy/computation identity, atomically audited lifecycle and snapshots, typed failures, and bounded abandoned-run recovery. The observation-only run and daemon commands update health independently of dashboard reads, coalesce one-minute input buckets, and drain/restart on SIGINT/SIGTERM without agent RPC or trust/node/scheduler mutation. |
