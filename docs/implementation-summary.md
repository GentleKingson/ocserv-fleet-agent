...
| 9. Optional Postgres backend | Runtime foundation | The default-off backend now has private DSN loading, a bounded pool, advisory-locked migrations, checksummed import/dry-run verification, doctor output, contract-preserving readers/writers, and fenced controller leases. SQLite and the SQLite-only API remain the defaults. |
| 10. Controlled writes | D0 state foundation | Schema 26 and the default-off module add signed-intent verification, replay-safe requests, dry-run policy decisions, expiring two-person approvals, cancellation, and atomic low-sensitive transition audit. The agent still rejects every write RPC and no reload/restart/apply adapter exists. |
