...
| 8. Store abstraction | Operationally mature A1 writer slice; implemented A2 storage slice | Added `StoreReader`, `StoreWriter`, `MigrationManager`, and `AuditWriter`; bounded SQLite reads fail closed on contaminated dynamic JSON. Every production mutation family in the A1 inventory uses actor-bearing atomic writers. Compare-before checks preserve concurrent decisions, external delivery I/O stays outside transactions, and the guard rejects fixture/raw bypasses from production. Schema 9 makes scheduler selector/pair writes closed and versioned, migrates or quarantines exact legacy rows, and makes both CLI/API readers reject contamination. API retains a narrower independent read adapter. |
