...
| 9. Optional Postgres backend | Runtime foundation | The default-off backend now has private DSN loading, a bounded pool, advisory-locked migrations, bounded checksummed import/dry-run/export verification, explicit `postgres doctor|import|export` commands, contract-preserving readers/writers, and transactionally fenced controller leases. Unencrypted connections fail closed outside Unix sockets/loopback. SQLite and the SQLite-only API remain the defaults. |
