...
| 9. Optional Postgres backend | Runtime foundation | The default-off backend now has private DSN loading, a bounded pool, advisory-locked migrations, bounded checksummed import/dry-run verification, doctor output, contract-preserving readers/writers, and transactionally fenced controller leases. Unencrypted connections fail closed outside Unix sockets/loopback. SQLite and the SQLite-only API remain the defaults. |
| 10. Controlled writes | D0 state foundation | Schema 28 and the default-off module add private trusted-key resolution, actor/reason-bound signed intents, replay-safe requests, dry-run policy decisions, durable expiry, explicit terminal transitions, expiring two-person approvals, cancellation, and atomic low-sensitive transition audit. The agent still rejects every write RPC and no reload/restart/apply adapter exists. |
