...
| 8. Store abstraction | Operationally mature A1 writer slice; implemented A2 storage slices | Added `StoreReader`, `StoreWriter`, `MigrationManager`, and `AuditWriter`; bounded SQLite reads fail closed on contaminated dynamic JSON. Every production mutation family in the A1 inventory uses actor-bearing atomic writers. Schemas 9 through 17 close scheduler selector/pair, health snapshot, observation-summary, run-summary, trust-bundle, alert-detail, webhook-host-allow, enrollment-metadata, and delivery-attempt storage, migrate exact legacy rows, and make readers reject contamination without exposing storage envelopes. API retains a narrower independent read adapter. |
