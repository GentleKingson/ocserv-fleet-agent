...
| 8. Store abstraction | Implemented slice, expansion active | Added `StoreReader`, `StoreWriter`, `MigrationManager`, and `AuditWriter`; bounded SQLite reads fail closed on contaminated dynamic JSON. Node add/enable/disable/remove now use actor-bearing atomic writer methods with audit-failure rollback tests. API retains a narrower independent read adapter and scheduler/retention/alert writers are not all migrated. |
...
- Migrate remaining scheduler, retention, and alert mutations to the
