...
The first A7 slice defines a closed controller metric catalog and exposes it at
the existing read-only API `/metrics` route. Scrapes share loopback-default and
...
| ID | Issue | Status | Depends on | Release target | Acceptance and required evidence |
| --- | --- | --- | --- | --- | --- |
| B1 Health history, rollups, and SLOs | `#41` | operationally mature | A2, A4 | `v0.3.x` | Schema 23 history, schema 24 reproducible rollups, schema 25 one-status-per-slot semantics, bounded 24h/7d/30d CLI/API projections, deterministic closed-bucket refresh, independent retention, and hardened timer operation are complete. `docs/b1-health-history-rollup-slo-inventory.md` records the evidence. |
| B2 Labels, environment, and maintenance | `#42` | operationally mature | A1, A2, A3 | `v0.4.0` | Schema 26 adds bounded audited metadata, exact selectors with a 50-node cap, per-node maintenance scheduling, advisory trust-policy diffs, CLI/API projections, and strict separation from trust, peers, and path authorization. `docs/b2-node-metadata-inventory.md` records the evidence. |
| B3 Versioned API and query contract | `#43` | operationally mature | A2, B1, B2 | `v0.4.0` | `/api/v1` has exact fleet summary, node and alert singles, signed bounded keyset pagination, time/metadata/reason filters, ETag/304, correlated stable errors, exact OpenAPI/runtime validation, explicit legacy compatibility, and no RPC-trigger route. `docs/b3-api-v1-inventory.md` records the evidence. |
| B4 Local producer SDK | `#44` | operationally mature | A2 | `v0.4.0` | `ocfleet-snapshot-schema`, its validator, Rust atomic producer SDK, embedded machine schema, fail-closed version check, agent compatibility suite, sample, author guide, and non-root Linux permission tests emit fixed aggregates only and remain unreachable from controller RPC/API/scheduler paths. `docs/b4-snapshot-producer-inventory.md` records the evidence. |
| B5 HMAC configuration fingerprint | `#45` | operationally mature | A2, B4 | `v0.4.0` | Agent-local HMAC-SHA-256 uses owner-only bounded key files, stable key IDs, explicit current/previous dual reporting, rotation-aware drift evaluation, redacted errors/debug output, and shortened CLI/storage projections. Explicit legacy SHA-256 preserves the old response shape. Linux permission, symlink, hardlink, stability, key separation, rotation, and compatibility tests pass. `docs/b5-hmac-config-fingerprint-inventory.md` records the evidence. |
| B6 Trust policy GitOps | `#46` | operationally mature | A1, A2 | `v0.4.0` | Detached Ed25519 revisions with pinned keys feed canonical deterministic 512-change plans, private JSON/Markdown CI artifacts, a closed drift-alert projection, signed actor-bound approvals, and a bounded append-only history. Immutable review rejects migration/active WAL and never creates missing state. No apply, RPC, enrollment, trust, peer, or path mutation exists. `docs/b6-trust-policy-gitops-inventory.md` records the evidence. |
| B7 Protocol capability negotiation | `#47` | operationally mature | A2, B4, B5 | `v0.4.0` | Fixed `node.capabilities` returns bounded closed protocol/method/provider/feature data without paths, commands, units, secrets, or local policy detail. Controller incompatibility fails closed, legacy `METHOD_NOT_FOUND` is deterministic, audits reduce raw lists to counts, and negotiation never enables actions. `docs/b7-protocol-capabilities-inventory.md` records the cross-crate and Docker evidence. |
| B8 Agent version drift | `#48` | operationally mature | A4, B2, B7 | `v0.4.0` | Schema 27 records latest closed capability observations atomically with RPC audit. Bounded SemVer/pre-release policy, distribution, outdated/protocol/provider alerts, readiness, API/dashboard projection, unknown handling, and redaction pass without any install, package-manager, restart, or automatic upgrade path. `docs/b8-agent-version-governance-inventory.md` records the evidence. |
| B-READY Read-only product gate | n/a | operationally mature | B1, B2, B3, B4, B5, B6, B7, B8 | `v0.4.0` | All Stage B issues are operationally mature. Bounded query/history and mixed-version behavior pass default/all-feature workspace gates plus non-root network-isolated Linux checks, without expanding trust or agent control. `docs/b-ready-readonly-product-gate.md` records the milestone evidence and remaining release boundary. |
...
| ID | Issue | Status | Depends on | Release target | Acceptance and required evidence |
| --- | --- | --- | --- | --- | --- |
| D0 Dry-run and approval state machine | `#52` | implemented | A-READY; live dispatch also requires C2 | `controlled-writes-preview` | Schema 28 and feature-gated CLI enforce different actor/approver, expiring approval, exact EndpointID, digest, UUID, idempotency, replay protection, private actor-bound signed-intent trust, default-deny local policy, successful prior dry-run, and atomic bounded audit. Feature-enabled agents still perform no mutation in D0. |
