...
The current execution node is the **A5 completion audit** on issue `#37`. A1 through A4 are
...
| A5 Safe alert delivery worker | `#37` | completion audit active | A1, A2, A3, A4 | `v0.3.0` | Fixed JSONL/HTTPS delivery supports per-hook control, claim, bounded retry, dead-letter, recovery, grouping, rate limit, delivery health, idempotency, history, and shutdown. SSRF, HMAC, no-redirect, secret-redaction, and forbidden command/script/template tests pass. `docs/a5-alert-delivery-inventory.md` records the completion evidence. |
