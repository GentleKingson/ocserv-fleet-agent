...
| Health | partially implemented / active implementation | `ocfleet health summary`, `ocfleet health node`, `ocfleet health snapshot list`, `ocfleet health policy show/set`; computed snapshots and their evaluation audit commit as one bounded transaction |
| Alerts | partially implemented / active implementation | Candidate evaluation and silence/resolve use stale-before checks and atomic audit; webhook-hook creation is atomic and replay-safe. `ocfleet alert list`, hook management, test/deliver, JSONL, and explicit HTTPS webhook delivery remain available; delivery persistence still needs its own durable boundaries. |
