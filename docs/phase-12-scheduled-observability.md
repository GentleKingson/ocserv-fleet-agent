...
| Alerts | partially implemented / active implementation | Candidate evaluation commits its bounded alert batch and audit atomically with stale-before conflict checks; `ocfleet alert list --state ... --severity ... --node ... --json`, hook management, test/deliver/silence/resolve, JSONL, and explicit HTTPS webhook delivery remain available |
